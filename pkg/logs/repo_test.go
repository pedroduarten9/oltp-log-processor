...
func TestIncrementAttributeAndReset(t *testing.T) {
	repo := NewRepo()
	...
	for _, tt := range tests {
		...
		assert.Empty(t, repo.counts)
	}
}

/// Benchmarks

func BenchmarkRepo_Serial(b *testing.B) {
	key := "foo"
	repo := NewRepo()
...
func BenchmarkRepo_Parallel(b *testing.B) {
	key := "foo"
	repo := NewRepo()
...
func BenchmarkRepo_ManyKeys(b *testing.B) {
	repo := NewRepo()
...
func BenchmarkRepo_WriteAndReset(b *testing.B) {
	repo := NewRepo()
...
func BenchmarkRepo_ConcurrentWriteAndRead(b *testing.B) {
	key := "foo"
	repo := NewRepo()
